...
<script setup lang="ts">
  ...
  export interface MenuItemType {
    ...
    /** 菜单项是否禁用 */
    disabled?: boolean
    /** 菜单项类型。error/danger 会渲染为危险操作样式 */
    type?: 'primary' | 'success' | 'warning' | 'danger' | 'error' | 'info'
    /** 兼容旧操作配置，true 时等同于 type=error */
    danger?: boolean
    /** 菜单项是否显示分割线 */
    showLine?: boolean
    /** 子菜单 */
  ...
  const isDangerItem = (item: MenuItemType) => {
    return item.type === 'error' || item.type === 'danger' || item.danger === true
  }
...
<style scoped>
  ...
  .menu-item.is-danger:not(.is-disabled) .menu-label,
  .menu-item.is-danger:not(.is-disabled) :deep(.art-svg-icon) {
